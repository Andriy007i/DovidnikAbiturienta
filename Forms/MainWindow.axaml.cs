        ...
        public MainWindow()
{
    ...
    if (File.Exists(SaveFile))
    {
        try
        {
            ...
            var loadedUniversities = JsonSerializer.Deserialize<List<University>>(json, options);
            if (loadedUniversities != null && loadedUniversities.Any())
            {
                Console.WriteLine($"Завантажено {loadedUniversities.Count} університетів");
                foreach (var uni in loadedUniversities)
                {
                    Console.WriteLine($"Університет: {uni.Name}, Спеціальностей: {uni.Specialties.Count}");
                }
                _repository.GetAllUniversities().Clear();
                _repository.GetAllUniversities().AddRange(loadedUniversities);
            }
            else
            {
                Console.WriteLine("Завантажені дані порожні або null");
            }
        }
        catch (Exception ex)
        {
        ...
        private void ResultsListBox_SelectionChanged(object? sender, SelectionChangedEventArgs e)
{
    ...
    if (ResultsListBox.SelectedItem != null)
    {
        ...
        SpecialtyDetailsTextBlock.BringIntoView();
    }
}
        ...
        private void UpdateSpecialtyDetails()
        {
            ...
            if (specialty != null)
            {
                SpecialtyDetailsTextBlock.Text =
                                                "Дані про обрану спеціальність:\n" +
                                                $"Назва: {specialty.Name}\n" +
                                                $"Код: {specialty.Code}\n" +
        ...
        private void SaveData()
        {
            try
            {
                var universities = _repository.GetAllUniversities();
                if (universities == null)
                {
                    Console.WriteLine("Список університетів є null.");
                    return;
                }
                ...
                var json = JsonSerializer.Serialize(universities, options);
                File.WriteAllText(SaveFile, json);
                Console.WriteLine($"Дані успішно збережено у {SaveFile}");
            }
